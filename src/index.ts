...
export class Relogs {
  ...
  constructor(relogsConfigs: RelogsConfigs) {
    ...
    if ((relogsConfigs.workspaceId?.length === 0)
      || (relogsConfigs.apiKey?.length === 0)) {
      console.error("No workspace id/api key");
      return;
    }

    if ((relogsConfigs.ingestConsoleLogs)
      && !relogsConfigs.defaultTable) {
    ...
    this.flushIntervalId = setInterval((() => { this.flushAsync() }) as Function, 1000);
  }
  ...
  async end() {
    ...
    await waitUntil(() => ((this.eventsBuffer.length === 0)
      && (this.uploadJobsInProgress === 0)), { timeout: 10 * 1000/*ms*/ });
  }
  ...
  async flushAsync() {
    ...
    for (const [tableName, events] of Object.entries(eventsByTables)) {
      await this.sendEventAsync((events as Event[]).map((e: Event) => e.eventData), tableName);
    }
    this.uploadJobsInProgress--;
  }
